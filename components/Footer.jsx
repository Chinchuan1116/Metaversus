    ...
    <div className={`${styles.innerWidth} mx-auto flex flex-col gap-8`}>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <p className="font-normal text-[14px] text-white opacity-50">Copyright @ 2023 Metaversus. All right reserved.</p>
          <div className="flex gap-4">{socials.map((social) => (<img src={social.url} key={social.name} alt={social.name} className="w-[24px] h-[24px] object-contain cursor-pointer" />))}</div>
        </div>
      </div>
    </div>
  </motion.footer>
